...
//    }
    ...
    diffuseTexture_=new QOpenGLTexture(QImage(":/image/images/bricks2.jpg").mirrored());
    normalMapTexture_=new QOpenGLTexture(QImage(":/image/images/bricks2_normal.jpg").mirrored());
    heightTexture_=new QOpenGLTexture(QImage(":/image/images/bricks2_disp.jpg").mirrored());
...
void NormalMapPlaneModel::Draw(QOpenGLShaderProgram &shader) {
    ...
    shader.setUniformValue("heightMap", 2);
    ...
    glFuns_->glBindTexture(GL_TEXTURE_2D, heightTexture_->textureId());
    renderQuad();
