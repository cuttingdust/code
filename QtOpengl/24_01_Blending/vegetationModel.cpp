...
std::vector<QVector3D> gVegetationPoints;

void VegetationModel::processNode(aiNode *node, const aiScene *scene)
{
    gVegetationPoints.push_back(QVector3D(-1.5f, 0.0f, -0.48f));
    gVegetationPoints.push_back(QVector3D(1.5f, 0.0f, 0.51f));
    gVegetationPoints.push_back(QVector3D(0.0f, 0.0f, 0.7f));
    gVegetationPoints.push_back(QVector3D(-0.3f, 0.0f, -2.3f));
    gVegetationPoints.push_back(QVector3D(0.5f, 0.0f, -0.6f));


    for (auto pos :gVegetationPoints) {
...
void VegetationModel::Draw(QOpenGLShaderProgram &shader) {
    ...
    {
        QMatrix4x4 model;
        model.translate(gVegetationPoints[i]);
        shader.bind();
