...
void TriangleModel::processNode(aiNode *node, const aiScene *scene)
{
    triangleMesh_ = processMesh(nullptr, nullptr);
}

Mesh* TriangleModel::processMesh(aiMesh *mesh, const aiScene *scene)
{
    std::vector<Vertex> vertices(36);
    std::vector<unsigned int> indices;
    std::vector<Texture> textures;

    int iSize = sizeof( gTriangleVertices ) / sizeof( float );
...
TriangleModel::TriangleModel(QOpenGLFunctions_4_1_Core *glfuns):
    glFuns_(glfuns)
{
...
void TriangleModel::Draw(QOpenGLShaderProgram &shader) {
    QMatrix4x4 model;
    shader.bind();
    shader.setUniformValue("model", model);
    shader.setUniformValue("skybox", 15);
    triangleMesh_->Draw(shader);
}
