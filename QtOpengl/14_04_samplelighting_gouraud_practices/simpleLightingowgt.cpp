...
void SimpleLightingOWgt::paintGL()
{
    ...
    gLightPos.setX(cos(time / 50) * 2.5f);
    gLightPos.setY(sin(time / 50) * 2.5f);
    glBindVertexArrayAPPLE(VAO);
    glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, EBO);
    ...
    shaderProgram_.setUniformValue("objectColor",gLightColor);
    shaderProgram_.setUniformValue("lightColor",gObjectColor);
