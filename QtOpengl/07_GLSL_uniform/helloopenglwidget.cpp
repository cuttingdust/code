...
{
    auto bSuc = connect(&timer_, SIGNAL(timeout()), this, SLOT(slotTimeout()));
    timer_.start(100);
}
...
void HelloOpenGLWidget::slotTimeout() { 
    makeCurrent();
    ...
    int timeVaule = QTime::currentTime().second();
    float greenVaule = (sin(timeVaule) / 2.0f) + 0.5f;
    shaderProgram_.setUniformValue("ourColor", 0.0f, greenVaule, 0.0f, 0.0f);
