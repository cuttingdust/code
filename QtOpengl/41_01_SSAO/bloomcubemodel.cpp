...
//    auto diffuseTexture = new QOpenGLTexture(QImage(":/image/texMap/container2.png").mirrored());
    auto specularTexture = new QOpenGLTexture(QImage(":/image/texMap/container2.png").mirrored());

    QImage wall = QImage(":/image/texMap/container2.png").convertToFormat(QImage::Format_RGB888);
...
//    textures.push_back(tex);
    ...
    glFuns_->glGenTextures(2, colorBuffers);
    for (unsigned int i = 0; i < 2; i++)
    {
        glFuns_->glBindTexture(GL_TEXTURE_2D, colorBuffers[i]);
        ...
        glFuns_->glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0 + i, GL_TEXTURE_2D, colorBuffers[i], 0);
    }
...
// --------------------------------------------------
    ...
    for (unsigned int i = 0; i < amount; i++)
    {
        ...
        glFuns_->glBindTexture(GL_TEXTURE_2D, first_iteration ? colorBuffers[1] : pingpongColorbuffers[!horizontal]);  // bind texture of other framebuffer (or scene if first iteration)
        renderQuad();
...
void BloomCubeModel::DrawBloom(QOpenGLShaderProgram &shader) {
    ...
    glFuns_->glBindTexture(GL_TEXTURE_2D, colorBuffers[0]);
...
float BloomCubeModel::getExposure() const {
    return exposure_;
}

void BloomCubeModel::setExposure(float exposure) {
    exposure_ = exposure;
}
