...
void ModelLoadingOWgt::enableGl() {
...
void ModelLoadingOWgt::bindFramBuffers() {
    if (aGlass_ && bDrawFramBuffer_)
    {
        aGlass_->bindFramer();
    }
...
//            glCullFace(GL_BACK);
        ...
        if (!modelsMap_.empty() && !bDeferedRender_ && !bDrawSSAO_)
        {
            shaderProgram_.bind();
...
void ModelLoadingOWgt::createShaderProgram() {
    ...
    ////////////////////////////////
    geometryPassShaderProgram_.addShaderFromSourceFile(QOpenGLShader::Vertex, ":/ssao/ssao_gBuffer.vert");
    geometryPassShaderProgram_.addShaderFromSourceFile(QOpenGLShader::Fragment, ":/ssao/ssao_gBuffer.frag");
    bSucess = geometryPassShaderProgram_.link();
    if (!bSucess) {
        qWarning() << __FUNCTION__ << " " << geometryPassShaderProgram_.log();
    }

    lightingPassShaderProgram_.addShaderFromSourceFile(QOpenGLShader::Vertex, ":/ssao/SSAO.vert");
    lightingPassShaderProgram_.addShaderFromSourceFile(QOpenGLShader::Fragment, ":/ssao/SSAO_lighting.frag");
    bSucess = lightingPassShaderProgram_.link();
    if (!bSucess) {
        qWarning() << __FUNCTION__ << " " << lightingPassShaderProgram_.log();
    }

    SSAOShaderProgram_.addShaderFromSourceFile(QOpenGLShader::Vertex, ":/ssao/SSAO.vert");
    SSAOShaderProgram_.addShaderFromSourceFile(QOpenGLShader::Fragment, ":/ssao/SSAO.frag");
    bSucess = SSAOShaderProgram_.link();
    if (!bSucess) {
        qWarning() << __FUNCTION__ << " " << SSAOShaderProgram_.log();
    }

    blurrShaderProgram_.addShaderFromSourceFile(QOpenGLShader::Vertex, ":/ssao/SSAO.vert");
    blurrShaderProgram_.addShaderFromSourceFile(QOpenGLShader::Fragment, ":/ssao/ssao_blurr.frag");
    bSucess = blurrShaderProgram_.link();
    if (!bSucess) {
        qWarning() << __FUNCTION__ << " " << blurrShaderProgram_.log();
    }
}
...
void ModelLoadingOWgt::bindUBOBuffer() {
    ...
    int uniformBlockIndexNormalShader =
            glGetUniformBlockIndex(normalShaderProgram_.programId(),"Matrices");
    glUniformBlockBinding(geometryPassShaderProgram_.programId(),
                          uniformBlockIndexNormalShader, 0);
    ...
    int uniformBlockIndexGeometryPassShader =
            glGetUniformBlockIndex(geometryPassShaderProgram_.programId(),"Matrices");
    glUniformBlockBinding(geometryPassShaderProgram_.programId(),
                          uniformBlockIndexGeometryPassShader, 0);
...
void ModelLoadingOWgt::createModel(QOpenGLFunctions_4_1_Core *funcs) {
    ...
    aDepthMap_ = new DepthMapModel(funcs, width(), height());
    aBloomCube_ = new BloomCubeModel(funcs, width(), height());


    auto* backmodel = loadModel("/Users/handabao/MyFile/code/QtOpengl/40_01_DeferredShading/backpack/backpack.obj");
    aSSAOModel_ = new SSAOModel(funcs, width(), height(), backmodel);
...
bool ModelLoadingOWgt::isBDeferedRender() const {
    return bDeferedRender_;
}

void ModelLoadingOWgt::setBDeferedRender(bool bDeferedRender) {
    bDeferedRender_ = bDeferedRender;
}

bool ModelLoadingOWgt::isBDrawSsao() const {
    return bDrawSSAO_;
}

void ModelLoadingOWgt::setBDrawSsao(bool bDrawSsao) {
    bDrawSSAO_ = bDrawSsao;
}

void ModelLoadingOWgt::drawSSAOModel() {
 if (bDrawSSAO_) {
     glDisable(GL_BLEND);
     aSSAOModel_->Draw(geometryPassShaderProgram_);
     aSSAOModel_->releaseFramebuffer();

     aSSAOModel_->DrawSSAO(SSAOShaderProgram_, projection);
     aSSAOModel_->DrawBlur(blurrShaderProgram_);
     aSSAOModel_->DrawLighting(lightingPassShaderProgram_, view);
     glEnable(GL_BLEND);
 }
}
